...
namespace HttpMock.Unit.Tests
{
    ...
    public class RequestProcessorTests
    {
        ...
        public void SetUp()
        {
            ...
            _ruleThatReturnsFirstHandler.Stub(x => x.IsEndpointMatch(null, new HttpRequestHead { Headers = new Dictionary<string, string>() })).IgnoreArguments().Return(true).Repeat.Once();

            _ruleThatReturnsNoHandlers = MockRepository.GenerateStub<IMatchingRule>();
            _ruleThatReturnsNoHandlers.Stub(x => x.IsEndpointMatch(null, new HttpRequestHead { Headers = new Dictionary<string, string>() })).IgnoreArguments().Return(false);
        }
        ...
        public void Returns_mock_not_found_when_handler_constraints_cannot_be_verified()
        {
            ...
            p.OnRequest(new HttpRequestHead { Uri = "http://blah.com/cheese/" + excludePhrase, Headers = new Dictionary<string, string>() }, null, response);
